...
//    System.out.println(temp[2]); // error
    ...
    System.out.println("배열 그대로 출력하면 참조값 : " + intArray);

    intArray[1] = 77; // 인덱스 1번 원소 값을 변경
    System.out.println("변경 후 intArray[1] : " + intArray[1]);

    // * 배열 변수 선언과 대입 시점 다르다면, "new 타입[]"을 중괄호 앞에 붙여야 함
    char[] charArray;
//    charArray = {'A', 'B', 'C'};
    charArray = new char[]{'A', 'B', 'C'};
    System.out.println("chatArray[0] : " + charArray[0]);
    System.out.println("chatArray[1] : " + charArray[1]);
    System.out.println("chatArray[2] : " + charArray[2]);

    // - new 연산자로 배열 선언/생성
    // : 배열 원소 값은 기본 값으로 초기화됨
...
//    doubleArray.length = 5; // Error: length 필드는 읽기 전용. 수정 불가!
...
//    System.out.println(doubleArray[doubleArray.length]); // ArrayIndexOutOfBoundsException
    ...
    int[] copiedArr = Arrays.copyOf(originArr, 2);
    ...
    double average = (double) sum1 / avgArr.length;
